...
const PreparednessShield = ({ preparedness = 0, prepLevel = { level: "Rookie Guardian", color: "text-red-400" } }) => {
  const radius = 45;
  ...
  return (
    <motion.div
      ...
      role="region"
      aria-label="Preparedness Shield"
    >
      <div className="absolute inset-0 bg-gradient-to-br from-[#FF6F00]/10 to-[#0D47A1]/10 rounded-3xl pointer-events-none"></div>
      <div className="relative z-10 text-center">
        <h3 className="text-lg font-semibold text-[#B0B0B0] mb-4">Preparedness Shield</h3>

        {/* Radial Progress */}
        <div className="relative mx-auto w-32 h-32 mb-4" aria-hidden>
          <svg className="w-32 h-32 transform -rotate-90" viewBox="0 0 100 100" role="img" aria-label={`Preparedness ${preparedness} percent`}>
            {/* Background circle */}
            <circle
              cx="50"
              cy="50"
        ...
        {/* Shield Icon */}
        <div className="text-4xl mb-2 group-hover:scale-110 transition-transform duration-300" aria-hidden>
          🛡️
        </div>
...
const Dashboard = () => {
  ...
  const preparednessRef = useRef(preparedness);

  useEffect(() => {
    localStorage.setItem("preparedness", String(preparedness));
  ...
  useEffect(() => {
    ...
    const currentStreak = parseInt(localStorage.getItem("dailyStreak") || "0", 10);
    ...
    // intentionally no deps to run on mount only
  ...
  useEffect(() => {
    ...
    const fetchAlerts = async () => {
      try {
        ...
        const userLocation = (localStorage.getItem("userLocation") || "Delhi").toLowerCase();
        const severeAlert = (fetchedAlerts || []).find(
          (alert) =>
            alert &&
            alert.severity === "Severe" &&
            typeof alert.area === "string" &&
            alert.area.toLowerCase().includes(userLocation)
        );
        if (severeAlert) {
          setShowBeacon(true);
        } else {
        ...
        // don't crash the dashboard — just show nothing
      }
    };
  ...
  const handleLogin = useCallback(() => {
    if (inputName.trim() !== "") {
      ...
      toast.success(`Welcome to Aapda Setu, ${inputName.trim()}! 🎉`);
    } else {
      toast.error("Name can't be empty.");
    }
  }, [inputName]);

  const getPreparednessLevel = useCallback((p) => {
    if (p >= 90) return { level: "Guardian Elite", color: "text-emerald-400", ring: "ring-emerald-400" };
    if (p >= 70) return { level: "Safety Expert", color: "text-blue-400", ring: "ring-blue-400" };
    if (p >= 50) return { level: "Preparedness Pro", color: "text-yellow-400", ring: "ring-yellow-400" };
    if (p >= 25) return { level: "Safety Scout", color: "text-orange-400", ring: "ring-orange-400" };
    return { level: "Rookie Guardian", color: "text-red-400", ring: "ring-red-400" };
  }, []);

  const prepLevel = useMemo(() => getPreparednessLevel(preparedness), [preparedness, getPreparednessLevel]);
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: "#212121" }}>
      ...
      <div className="relative overflow-hidden" style={{ backgroundColor: "#0D47A1" }}>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="flex items-center justify-between mb-8">
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
              <h1 className="text-4xl font-bold text-white mb-2">
                Welcome back,{" "}
                <span className="bg-gradient-to-r from-[#FF6F00] to-[#FFA000] bg-clip-text text-transparent">
                  {username || "Guardian"}
                </span>
              ...
              {streakCount > 1 && (
                <p className="text-sm text-[#FF6F00] font-semibold mt-1">🔥 {streakCount} day streak! Keep it up!</p>
              )}
            </motion.div>
                ...
                {showProfile && (
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      <Link to="/profile" className="block w-full text-center py-2 px-4 bg-[#FF6F00]/20 text-[#FF6F00] rounded-lg hover:bg-[#FF6F00]/30 transition-colors font-medium">
                        View Profile
                      </Link>
                      <Link to="/settings" className="block w-full text-center py-2 px-4 bg-gray-700 text-[#B0B0B0] rounded-lg hover:bg-gray-600 transition-colors font-medium">
                        Settings
                      </Link>
                      <button onClick={handleLogout} className="w-full py-2 px-4 bg-[#D50000]/20 text-[#D50000] rounded-lg hover:bg-[#D50000]/30 transition-colors font-medium">
                        Logout
                      </button>
                    </div>
                  </motion.div>
              ...
              >
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className={`text-3xl font-bold ${alerts.length > 0 ? "text-[#D50000]" : "text-white"}`}>{alerts.length}</p>
          ...
          <motion.div
            ...
            role="button"
            aria-label="Watch today's safety video"
          >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12 pt-8">
        <motion.h2 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="text-3xl font-bold text-white mb-8">
          Your Missions 🎯
        </motion.h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {missionData.map((mission, index) => (
            <motion.div key={mission.title + index} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 + index * 0.1 }}>
              <Link to={mission.to} className="group relative bg-gradient-to-br from-gray-800/50 to-gray-900/50 backdrop-blur-xl border border-gray-700/50 rounded-2xl p-6 shadow-lg hover:scale-105 transition-all duration-300 overflow-hidden block">
                ...
                {mission.isActive && <div className="absolute top-4 right-4 w-3 h-3 bg-[#D50000] rounded-full animate-pulse" />}
                ...
                <div className="relative z-10">
                  <h3 className="text-lg font-bold text-white mb-2 group-hover:text-[#FF6F00] transition-colors">{mission.title}</h3>
                  <p className="text-[#B0B0B0] mb-4 leading-relaxed text-sm">{mission.description}</p>
                  ...
                  <div className="mb-4">
                    ...
                    <div className="w-full bg-gray-700 rounded-full h-2">
                      <div className="bg-gradient-to-r from-[#FF6F00] to-[#FFA000] h-2 rounded-full transition-all duration-300" style={{ width: `${mission.progress}%` }} />
                    </div>
                  </div>

                  <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="w-full bg-gradient-to-r from-[#FF6F00] to-[#FFA000] text-white py-2 px-4 rounded-lg font-semibold hover:from-[#FF8F00] hover:to-[#FFB300] transition-all duration-200 shadow-lg text-sm">
                    {mission.buttonText}
                  </motion.button>
                </div>
              </Link>
